...
### 5 September: automatic device handoff

Matthew confirmed manual Mac/Windows Cloud transfer and the healthy-source warning correction. Public Beta 25134966 now adds last-sync hash comparison and durable publication normalization, so an unchanged local checkpoint automatically accepts a validated downloaded primary. Independent local changes, unknown baselines and backup recovery retain a choice. Both snapshots are preserved. Existing devices may need one initial choice until each publishes a normalized checkpoint. Full suite 1,223 tests; lint/native build and packaged Mac close smoke pass. Windows/Linux/macOS published from bddd1912; default 22353758 unchanged. Seamless cross-machine acceptance remains pending. Details and manifest IDs are in 4.1.7-baseline-review.md.
