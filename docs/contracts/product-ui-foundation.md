...
timestamp. Any intentional Web-only first-run gameplay default must be listed
in `webFirstRunGameplayOverridePaths`, applied only when no player save exists,
and covered by an exact artifact-delta test. TypeScript must not hand-copy or
silently invent any other gameplay defaults. The artifact records its Unity
version, schema and generated-data provenance and passes Unity-to-Web normalized
parity before product UI consumes it.
