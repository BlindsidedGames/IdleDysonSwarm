...
- F-03, F-04, F-07 and F-08: timer, offline-time, auxiliary-economy and Stellar Sacrifice changes run through canonical event/lifecycle ordering. Tests cover individual caps, fractional uncapped Shoulders progress, exact-cost atomic sacrifice, interval-start Bot funding, produced-Bot deferral, replay safety, representative-boundary recalculation and deterministic one-day adaptive grouping. Adaptive Stored Time deliberately trades 100 ms equivalence for bounded processing: each representative interval uses its start rates and funding, then advances timers/research before the next group recalculates. Infinity scheduling projects the same start-funded Stellar debit so a gross Bot-production estimate cannot invent an internal funding boundary; already-ready and minimum-cycle-delayed Infinity cases retain their canonical ordering.
...
- The targeted one-day Stored Time performance run remained bounded at 4,096 representative groups. The pre-remediation grouped-path probe completed one day in 1.49 seconds; the final post-remediation test completed three one-day simulations (the existing baseline plus two deterministic timer/Stellar replays) in 3.69 seconds of test time, or about 1.23 seconds per simulation on the same development machine.
...
- Active ticks, short exact Stored Time spends and long adaptive Stored Time spends share the same within-interval ordering. Long spends remain an intentional approximation because multiple 100 ms ticks are represented by one bounded group; deterministic replay is guaranteed for the same grouping, not equality with an ungrouped 100 ms simulation.
