...
- The intended semantics for F-12 through F-20 are now recorded as approved design decisions, including the accepted Purity production cliff, Supernova disclosure requirements, Production Scaling's beyond-threshold boundary, VPT's authored-effect retargeting and Stellar Sacrifices' interval-start funding rule. This approval does not mean the gameplay repairs are implemented or verified; the affected runtime behavior, generated data, player-facing previews/effect breakdowns and boundary/integration/replay tests still need to be changed and validated as specified above.
