...
A browser build can import a Unity `IDB1` save supplied by file picker,
drag-and-drop or paste through the same decode, migrate, repair and validation
pipeline. While canonical mapping coverage and release writes remain gated, the
first slice stores that import in an explicitly isolated web-development
profile, retains the supplied original for recovery/export and does not
overwrite the player's Unity save. A browser release may promise complete save
migration only after the canonical mapping, authentic schema-12 round-trip and
release-write gates close.

Even after those gates close, a browser cannot promise automatic discovery of
an arbitrary Unity save path: browser filesystem access is sandboxed and
user-mediated. Electron and retained-container mobile upgrades may discover the
save automatically; browser players must choose or paste it once. All hosts
must satisfy the applicable persistence and lifecycle requirements in
...
checkpoint/reload tests proves continuity inside that isolated profile; it does
not certify canonical Unity-save overwrite or release migration.
...
13. Complete recovery/reload, localization, accessibility, supported-browser,
...
- [x] Electron desktop, Capacitor mobile and browser/PWA as product-capable
  hosts; browser Unity import is user-mediated and first-slice writes stay in an
  isolated development profile until canonical mapping and release-write
  certification close.
...
- [x] Browser/PWA enforces one writable owner, persistent-storage/quota
  ...
  behavior above.
- [x] The current/previous browser-engine policy and physical iOS/Android
  acceptance above.
