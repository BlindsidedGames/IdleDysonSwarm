...
The reference implementation writes Web-owned schema-13 `IDSWEB1` checkpoints.
`mappingCoverageManifest.coverageComplete` certifies one-way import of the
pinned public Unity surface; it is not a reverse-export gate. Passing
checkpoint/reconstruction tests proves Web-save continuity and does not claim a
Unity-readable export or two-way Unity synchronization.
