...
| Manual-building/Swarm scaling | `Assets/Scripts/Systems/ProductionMath.cs:53-62`; `Assets/Scripts/Systems/Stats/FacilityModifierPipeline.cs:518-617` | Unity applies the purchased-building threshold/divisor pipeline and Terra count substitution; Web lacks the whole layer. This confirms F-05/F-06/F-09. |
...
| Mirrored/orphan authoring | Generated catalogs and the same authored skill/effect assets | Terra Nova/Gloriae are dead in both runtimes. The unreferenced VPT Tinker x1.5 effect is orphaned in both and is not in VPT's displayed promise. Several copy/formula mismatches below are also mirrored legacy semantics, not exporter drift. |
...
| F-05 | Critical | Dead/no-op line | Super Swarm, Mega Swarm and Ultimate Swarm have no Web purchased-building production consumer. Production Scaling increments the shared fragment count but its advertised cutoff is unused. Toggle probe produced identical rates. Unity's layer uses cutoff 90/100 and divisors 50, integer `100/3`, or 20. | Up to eight points buy no advertised Swarm scaling on Web; Production Scaling can only help unrelated fragment-count formulas indirectly. | Port one canonical purchased-building scaling function into every facility rate, then resolve the mirrored copy/rounding issues in F-19. |
| F-06 | Critical | Dead/no-op/partial line | Web never applies Terra Firma/Eculeo/Infirma/Nullius planet-count substitution. Terra Irradiant is used only for a goal-progress/manual-planet path and VPT input, not the shared twelve-times count. Unity applies these five through its manual-building modifier layer. Terra Nova/Gloriae never enter planet price in either runtime. | The seven-point Terra route largely does nothing on Web; two price nodes are dead in the source game as well. | Port the shared Unity count layer for the five working nodes. Treat Nova/Gloriae as an upstream product decision: implement the prices in both runtimes or change/remove the promise. |
| F-07 | Critical | Dead auxiliary economy | Shoulders of Giants and What Could Have Been calculate `scienceBoostPerSecond`; Shoulders of the Enlightened calculates `moneyUpgradePerSecond`; Shoulder Surgery adds the Fallen bonus to What Could Have Been's Pocket-Dimension science-boost path. `canonicalDysonDerivation` exposes both rates under `auxiliary`, but `canonicalEventTimeModel` never applies them. Unity commits both rates every production tick. | The advertised science/cash-boost generation and Shoulder Surgery do not progress their upgrade levels on Web. | Add explicit event arrivals/commits for the two auxiliary rates, including offline/event-boundary behavior and caps. |
| F-08 | High | Confirmed partial bug | Web uses bots only as an eligibility threshold for Stellar Sacrifices; no per-second debit is emitted. Unity performs the debit after the same eligibility check. Stellar Dominance inherits the missing Web sacrifice and has a mirrored strict-boundary inconsistency. | Web players receive the planet upside without the documented ongoing bot cost; exact-threshold behavior is inconsistent. | Emit an atomic bot-debit/planet-credit event, cap the interval by affordable bots, and use one intended boundary consistently. |
| F-09 | High | Confirmed partial bug | Supernova's 1000x Stellar Sacrifices galaxy term reaches the Web planet formula, but Web has no manual-building modifier layer to disable. Unity excludes that layer while Supernova is owned. | The upside works on Web while the stated loss of manual-building production does not. | Port the manual-building layer and make Supernova disable it; verify whether costs/unlocks should still see manual purchases. |
| F-10 | High | Confirmed formula bug | Panel Warranty parses as `(5 * fragments > 1) ? 2^(fragments-1) : 1`, returning 1/2/4 in the probe. | The fragment lifetime bonus is much smaller than described. | Parenthesize and encode the documented base/doubling formula; test the exact examples printed in the UI. |
| F-11 | High | Confirmed formula bug | Regulated Academia uses `1.02 + 1.01 * (fragments - 1)` in both money and science paths. | One fragment supplies 2% rather than 20%; the second causes a discontinuous jump to 103%. | Express the intended percentage in one named helper and test fragment counts 1, 2, 3 and 7. |
...
| ID | Severity | Skills | Runtime versus description | Fix direction |
| --- | --- | --- | --- | --- |
| F-12 | Medium | Purity of Mind, Body, S-Essence | Dynamic formulas multiply by `coefficient * unspentPoints`, omitting a neutral `1 + ...`. They coincide with the advertised percentage only at one point and over-scale from two onward. | Decide additive bonus versus total multiplier; align code/text and test 0/1/2 points. |
| F-13 | Medium | Worker Boost, Produced as Science | Allocation percent is used directly as a multiplier; 1% allocation produces x1 instead of the documented +100% (x2). | Add the neutral base if copy is authoritative, or rewrite copy as a replacement multiplier. |
| F-14 | Medium | Citadel Council | Runtime adds `log base 1.2(panels decayed)` seconds to lifetime; text says lifetime is multiplied by that value. | Align operation and text; add a non-zero baseline case. |
| F-15 | Medium | Reapers | Runtime multiplier is `1 + log2(panels)/10`; text states `log2(panels)/10`. | Clarify whether the neutral +1 is intended and align the technical description. |
| F-16 | Medium | Worthy Sacrifice | Catalog multiplies Assembly modifier by 2.5 while text promises 5x; the 50% lifetime penalty is connected. | Change the authored value or the copy. |
| F-17 | Medium | Dyson Subsidies | Cash-before-star branch works, but assembly production switches only when `floor(stars) > 1`, leaving the first surrounded star without the promised 2x bot branch. | Use the intended first-star boundary and test values just below/at/above one. |
| F-18 | Medium | Galactic Pradigm Shift | Planet modifier switches to 3x only when `floor(galaxies) > 1`, not after the first engulfed galaxy as described. | Correct the threshold or copy and preserve the legacy-spelled skill ID. |
| F-19 | Medium | Production Scaling, Mega Swarm | The source manual-building layer makes Production Scaling a fixed cutoff of 90, ignoring the promised additional -5 per other fragment. Mega Swarm uses integer `100/3 = 33` as its divisor, yielding about 3.03% per step rather than exactly 3%. Web currently lacks the whole layer (F-05), so these are latent as well as mirrored. | Define the intended formula explicitly, avoid integer truncation, and add fragment-count plus 90/100-boundary cases before porting the layer. |
| F-20 | Medium | One Minute Plan, Pocket Androids, Stellar Obliteration, Versatile Production Tactics | One Minute Plan gives x1.5 at exactly 60s although copy says “otherwise” x5. Pocket Androids caps at 3,564s rather than one hour (latent behind F-03). Stellar Obliteration divides cash/science by ordinary galaxies although copy says Stellar Galaxies. VPT boosts AI Manager production (Assembly Line output), while copy says “produce 50% more AI Managers.” | Resolve each boundary/target term in source-of-truth copy and add exact transition/target tests. |
...
| Fragment count | Connected, with node defects | The canonical count is assigned fragment skills. It reaches fragment formulas, but Production Scaling's primary effect is absent; Panel Warranty and Regulated Academia formulas are wrong. |
...
| 012 | `dysonSubsidies` (1) | req `manualLabour` | Star-conditional money/assembly dynamic; first-star gap | D — F-17 |
...
| 017 | `galacticPradigmShift` (1) | req `dysonSubsidies` | Galaxy-conditional planet modifier; switch is after >1 galaxy | D — F-18 |
...
| 026 | `oneMinutePlan` (1) | req `worthySacrifice`, `dysonSubsidies` | Assembly modifier uses >60s, not the documented “otherwise” at 60s | D — F-20 |
...
| 047 | `versatileProductionTactics` (1) | req `oneMinutePlan` | Manager-production/Planet effects work; manager target wording is wrong | D — F-20 |
...
| 050 | `worthySacrifice` (1) | req `burnOut` | Lifetime penalty works; Assembly value is 2.5x, not 5x | D — F-16 |
...
| 056 | `pocketAndroids` (1) | req `solarBubbles`; excl `whatCouldHaveBeen` | Timer never advances; formula also caps at 3,564s, not one hour | N — F-03; F-20 |
...
| 075 | `productionScaling` (1) | req `superSwarm` | Counts as a fragment, but no advertised scaling-cutoff consumer | N — F-05; F-01 |
...
| # | Skill (cost) | Requirements | Actual Web consumer | Verdict |
| ---: | --- | --- | --- | --- |
| 079 | `purityOfMind` (2) | req `manualLabour` | Unspent-point money/science multipliers; missing neutral base | D — F-12; F-01 |
| 080 | `purityOfBody` (2) | req `purityOfMind` | Unspent-point Assembly modifier; missing neutral base | D — F-12; F-01 |
| 081 | `purityOfSEssence` (3) | req `purityOfBody` | Unspent-point global/facility multipliers; missing neutral base | D — F-12; F-01 |
...
| # | Skill (cost) | Requirements | Actual Web consumer | Verdict |
| ---: | --- | --- | --- | --- |
| 100 | `stellarSacrifices` (2) | root | Planet generation works; no per-second bot debit | B — F-08; F-01 |
...
| 103 | `stellarObliteration` (2) | req `stellarSacrifices` | Divides income by ordinary, not documented Stellar, galaxies | D — F-20; F-01 |
| 104 | `supernova` (4) | req `stellarObliteration` | 1000x stellar planet term works; manual-building exclusion absent | B — F-09; F-01 |
...
- This was investigation-only. No product code, tests, generated data, assets, configuration or Git state was intentionally changed.
- Runtime probes exercised canonical TypeScript functions, not a long interactive browser playthrough. The frontend structure and canonical preview paths were traced and covered by focused tests, but visual layout/localized rendering was not manually checked in every locale.
- Exact intended semantics for ambiguous legacy copy (especially F-12–F-18) require product/source-of-truth confirmation. The report distinguishes those mismatches from missing consumers.
- Unity catalog metadata and runtime behavior have been reconciled above. Shared legacy defects remain Web skill defects where they affect players, but they are explicitly distinguished from Web-only parity omissions.
