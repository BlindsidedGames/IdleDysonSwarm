...
import { resolveDynamicFacilitySkillEffect } from './dynamicFacilitySkillEffects'
import {
  ...
  try {
    ...
    const facility = resolveDynamicFacilitySkillEffect(effectId, {
      ...
      effectivePlanets:
        planets[0] +
        planets[1] *
          (ownedSkills.has('terraIrradiant') ? 12 : 1),
    ...
    const panelsPerSecond =
      tryResolvePanelsPerSecondDynamicEffect(effectId, panelInputs)
