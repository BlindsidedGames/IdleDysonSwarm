...
import { commitV2Purchase, quoteV2FixedPriceBuyMax, quoteV2Purchase, selectV2PurchaseBatches, type V2PurchaseMode } from './transactionsV2'
...
export interface DreamInfluencePurchaseResultV2{readonly accepted:boolean;readonly changed:boolean;readonly state:CanonicalGameStateV2;readonly batches:GameDecimal;readonly unitsGranted:GameDecimal;readonly quotedCost:GameDecimal;readonly debited:GameDecimal}
export function purchaseDreamInfluenceV2(state:CanonicalGameStateV2,id:DreamInfluencePurchaseIdV2,mode:V2PurchaseMode):DreamInfluencePurchaseResultV2{
  if(!admit(state)||!['hunters','gatherers','solar','fusion'].includes(id)||!['buy-1','buy-10','buy-50','buy-100','buy-max'].includes(mode))return influenceFailure(state)
  ...
  if(isZeroGameDecimal(price)||isZeroGameDecimal(units))return influenceFailure(state)
  ...
  if(!quote.accepted)return influenceFailure(state)
  const committed=commitV2Purchase(quote,{revision:0,balance:state.reality.influence,output:state.dream.resources[resource]});if(!committed.accepted)return influenceFailure(state)
  ...
  return Object.freeze({accepted:true,changed:committed.changed,state:candidate,batches:quote.batches,unitsGranted:quote.unitsGranted,quotedCost:quote.quotedCost,debited:committed.debitedAmount})
}
...
function influenceFailure(state:CanonicalGameStateV2):DreamInfluencePurchaseResultV2{return Object.freeze({accepted:false,changed:false,state,batches:GAME_DECIMAL_ZERO,unitsGranted:GAME_DECIMAL_ZERO,quotedCost:GAME_DECIMAL_ZERO,debited:GAME_DECIMAL_ZERO})}
