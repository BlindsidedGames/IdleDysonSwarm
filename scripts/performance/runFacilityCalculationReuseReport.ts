...
const referenceCommit = 'e71571199117ef4bc2e0c044b788cf21f9af65ab'
const root = resolve(import.meta.dirname, '../..')
const directory = mkdtempSync(resolve(tmpdir(), 'ids-facility-reference-'))
const referencePath = resolve(directory, 'reference.mts')
const referenceDysonPath = resolve(directory, 'dysonModel.mts')
function historicalSource(file: string) {
  return execFileSync('git', ['show', `${referenceCommit}:src/simulation/${file}`], {
    cwd: root, encoding: 'utf8',
  }).replace(
    /from '([.][^']+)'/g,
    (_match, specifier: string) => `from ${JSON.stringify(pathToFileURL(
      specifier === './dysonModel'
        ? referenceDysonPath
        : resolve(root, 'src/simulation', specifier),
    ).href)}`,
  )
}
writeFileSync(referenceDysonPath, historicalSource('dysonModel.ts'))
writeFileSync(referencePath, historicalSource('canonicalDysonDerivation.ts'))
...
try {
  ...
  console.log(JSON.stringify({
    ...
    referenceCommit,
    note: 'Checkpoint derivation and model implementations with current unchanged dependencies, isolating within-call facility rate calculation reuse. Differences near measurement noise are not speedup evidence; no timing gate.',
