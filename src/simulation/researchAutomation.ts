...
export function selectCanonicalResearchPresentationFacts(
  ...
  return Object.freeze({
    ...
    maxed,
    automationActive:
      !state.discovery?.unlocked && !isResearchDisabledByChallenge(state) && state.infinity.automationUnlocked.research &&
...
export function runResearchAutomationTick(
  ...
  if (state.discovery?.unlocked || isResearchDisabledByChallenge(state) || !state.infinity.automationUnlocked.research) {
    return {
      state,
...
function previewPurchase(
  ...
  if (state.discovery?.unlocked || isResearchDisabledByChallenge(state)) return emptyPreview(definition.id, 'challenge-active')
