...
export function hasCompletedInfinityChallenge(state: Readonly<CanonicalGameStateV1>): boolean { return state.challenges?.blankSlateCompleted === true || state.challenges?.trialAndErrorCompleted === true || state.challenges?.noScienceCompleted === true }
...
export function validateInfinityChallenges(value: unknown): string | null {
  ...
  if ((c.noScienceCompleted !== undefined && typeof c.noScienceCompleted !== 'boolean') || (c.trialAndErrorCompleted !== undefined && typeof c.trialAndErrorCompleted !== 'boolean') || typeof c.unlocked !== 'boolean' || typeof c.blankSlateCompleted !== 'boolean' ||
      typeof c.hasEarnedGalvanizer !== 'boolean' || (c.active !== null && c.active !== 'blank-slate' && c.active !== 'trial-and-error' && c.active !== 'no-science') ||
  ...
  if (c.completionSeconds !== undefined) {
    ...
    for (const [id, seconds] of Object.entries(c.completionSeconds)) {
      if (!['blank-slate', 'trial-and-error', 'no-science'].includes(id) || typeof seconds !== 'number' || !Number.isFinite(seconds) || seconds < 0 ||
        (id === 'blank-slate' ? !c.blankSlateCompleted : id === 'trial-and-error' ? !c.trialAndErrorCompleted : !c.noScienceCompleted)) return 'Invalid challenge completion time.'
    }
  }
  if (c.galvanizedSkillIds !== undefined) {
    ...
    if (!Array.isArray(ids) || ids.some((id) => typeof id !== 'string' || !valid.has(id)) ||
        new Set(ids).size !== ids.length || (ids.length > 0 && c.blankSlateCompleted !== true && c.trialAndErrorCompleted !== true && c.noScienceCompleted !== true)) {
