...
export type { CanonicalDevelopmentAction } from './canonicalDevelopmentCommands'
...
> {
  ...
  const engine: SimulationEngineDefinition<CanonicalRuntimeState, CanonicalApplicationCommand> = {
    ...
    applyCommand: (candidate, command) => {
      if (command.kind === 'challenge.enter-trial-and-error' || command.kind === 'challenge.enter-blank-slate' || command.kind === 'challenge.abandon') {
        ...
        const reset = restartInfinityChallenge(candidate.gameState,
          command.kind === 'challenge.abandon' ? 'abandon' : 'enter', artifact.value, command.kind === 'challenge.enter-trial-and-error' ? 'trial-and-error' : 'blank-slate')
