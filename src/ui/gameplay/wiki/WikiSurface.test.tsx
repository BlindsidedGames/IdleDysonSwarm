...
describe('WikiSurface', () => {
  ...
  test('keeps Patch Notes and earned Wiki Lore available before progression unlocks', async () => {
    ...
    expect(within(mostRecent!).getByRole('heading', { name: 'Version 3' })).toBeVisible()
    expect(within(mostRecent!).getAllByRole('listitem')).toHaveLength(13)
    expect(within(mostRecent!).getByText(/available to play on the Web/)).toBeVisible()
    expect(within(mostRecent!).getByText(/Offline Time with Stored Time/)).toBeVisible()
    expect(within(mostRecent!).getByText(/three rotating backups/)).toBeVisible()
    expect(within(mostRecent!).getByText(/installed as an app/)).toBeVisible()
    expect(within(mostRecent!).getByText(/Web Store with five/)).toBeVisible()
    expect(within(mostRecent!).getByText(/Expanded the Wiki/)).toBeVisible()
    ...
    expect(screen.getByRole('heading', { name: 'Previous' })).toBeVisible()
