...
describe('WikiSurface', () => {
  ...
  test('keeps Patch Notes and earned Wiki Lore available before progression unlocks', async () => {
    ...
    expect(within(mostRecent!).getByRole('heading', { name: 'Version 4.1.5' })).toBeVisible()
    expect(within(mostRecent!).getAllByRole('listitem')).toHaveLength(12)
    expect(within(mostRecent!).getByText(/Refined the entire interface/)).toBeVisible()
    expect(within(mostRecent!).getByText(/Automatic Influence gathering is continuous/)).toBeVisible()
    expect(within(mostRecent!).getByText(/timer edge case that could stop game processing/)).toBeVisible()

    const previous = screen.getByRole('heading', { name: 'Older' })
      .closest('section')
    expect(previous).not.toBeNull()
    expect(within(previous!).getByRole('heading', { name: 'Version 4.1.4' })).toBeVisible()
    expect(within(previous!).getByText(/no longer resets to one day/)).toBeVisible()
    expect(within(previous!).getByText(/former 9.22 quintillion limit/)).toBeVisible()
    expect(within(previous!).getByRole('heading', { name: 'Version 4.1.3' })).toBeVisible()
    expect(within(previous!).getByText(/step-by-step production breakdown/)).toBeVisible()
    expect(within(previous!).getByText(/Scientific Planets and Pocket Dimensions/)).toBeVisible()
    expect(within(previous!).getByText(/single-line rates/)).toBeVisible()
    expect(within(previous!).getByText(/continuous production after a long press/)).toBeVisible()
    expect(within(previous!).getByText(/Offline Time usage section/)).toBeVisible()
    expect(within(previous!).getByRole('heading', { name: 'Version 4.1.2' })).toBeVisible()
    expect(within(previous!).getByText(/French, German, Latin American Spanish/)).toBeVisible()
    expect(within(previous!).getByText(/Rebuilt active and Offline Time processing/)).toBeVisible()
    expect(within(previous!).getByText(/completion summary after each run/)).toBeVisible()
    expect(within(previous!).getByText(/Game Processing setting from 33–200 ms/)).toBeVisible()
    expect(within(previous!).getByText(/all game systems at twice their normal speed/)).toBeVisible()
    expect(within(previous!).getByRole('heading', { name: 'Version 4.1.1' })).toBeVisible()
    ...
    expect(within(previous!).getByRole('heading', { name: 'Version 4.1' })).toBeVisible()
    ...
    expect(within(previous!).getByRole('heading', { name: 'Version 4' })).toBeVisible()
    expect(within(previous!).getAllByRole('listitem')).toHaveLength(43)
    ...
    expect(within(earlier!).queryByRole('heading', { name: 'Version 4' }))
      .not.toBeInTheDocument()
