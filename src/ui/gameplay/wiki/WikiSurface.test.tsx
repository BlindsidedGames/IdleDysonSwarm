...
describe('WikiSurface', () => {
  ...
  test('keeps Patch Notes and earned Wiki Lore available before progression unlocks', async () => {
    ...
    expect(within(mostRecent!).getByRole('heading', { name: 'Version 4.1.4' })).toBeVisible()
    expect(within(mostRecent!).getAllByRole('listitem')).toHaveLength(2)
    expect(within(mostRecent!).getByText(/no longer resets to one day/)).toBeVisible()
    expect(within(mostRecent!).getByText(/former 9.22 quintillion limit/)).toBeVisible()
    ...
    expect(within(previous!).getAllByRole('listitem')).toHaveLength(41)
