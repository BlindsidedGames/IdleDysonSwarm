...
import { isMegaStructureFacility } from '../../../simulation/dysonFacilityCatalog'
import {
  Button,
  FacilityCard,
} from '../../components'
...
} from './facilityPresentationCatalog'
import './facilities.css'

export type DysonFacilityId = CanonicalFacilityId

export type FacilityPurchasePreview =
  FrontendGameplayPreviews['dyson']['facilities'][number]

export type FacilityCanonicalFact =
  Extract<
...
const facilityRootSkill: Readonly<Record<DysonFacilityId, string>> = {
  assembly_lines: 'assemblyLineTree',
  ai_managers: 'aiManagerTree',
  servers: 'serverTree',
  data_centers: 'dataCenterTree',
  planets: 'planetsTree',
  matrioshka_brains: 'research.matrioshka_brains_upgrade',
  birch_planets: 'research.birch_planets_upgrade',
  galactic_brains: 'research.galactic_brains_upgrade',
}
...
export interface FacilityPresentationCardProps {
  ...
  readonly preview: FacilityPurchasePreview
  readonly routeAvailable: boolean
  readonly pending: boolean
  readonly feedback?: FacilityPurchaseFeedback
...
export function FacilityPresentationCard({
  ...
  onPurchase,
  onOpenDetails,
}: FacilityPresentationCardProps) {
  const intl = useIntl()
  const feedbackId = useId()
  const availabilityId = useId()
  const presentation = facilityPresentation[facilityId]
        ...
        <div className="basic-facility-card__actions">
          <Button
            ...
            aria-label={intl.formatMessage(
              ...
              {
                facility: intl.formatMessage(presentation.name),
                prompt: intl.formatMessage(presentation.purchasePrompt),
...
export function FacilityDetailsContent({
  locale,
  facilityId,
  fact,
  gameSpeed,
...
type FacilityContribution = NonNullable<
  FacilityCanonicalFact['details']['contributions']
>[number]
...
function facilityIcon(facilityId: DysonFacilityId): string {
  if (isMegaStructureFacility(facilityId)) return navigationAssets.quantum
  return skillIcons[facilityRootSkill[facilityId]] ?? navigationAssets.bots
}
...
function disabledReason(
  preview: FacilityPurchasePreview,
...
function feedbackMessage(
  feedback: 'pending' | FacilityPurchaseFeedback['state'],
): MessageDescriptor {
  switch (feedback) {
    case 'pending':
...
function buttonState(
  feedback: 'pending' | FacilityPurchaseFeedback['state'] | undefined,
