...
describe('Avocato Overflow confirmation', () => {
  test('requires confirmation, allows cancellation, and prevents repeated dispatch while saving', async () => {
    ...
    expect(dispatch).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'Transcend for 1 point' }))
    const confirm = screen.getByRole('button', { name: 'Confirm Transcendence' })
    ...
    expect(screen.queryByRole('button', { name: 'Confirm Transcendence' })).toBeNull()
  })
  ...
  test('shows a failed save without claiming a reward and allows retry', async () => {
    ...
    setup(dispatch)
    fireEvent.click(screen.getByRole('button', { name: 'Transcend for 1 point' }))
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'Confirm Transcendence' })) })
    expect(screen.getByRole('alert').textContent).toContain('Your current run has been kept')
    expect(screen.getByRole('heading', { name: /Transcendence Points: 0/ })).not.toBeNull()
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'Confirm Transcendence' })) })
  ...
  test('keeps the reset disabled until the canonical boundary is reached', () => {
    ...
    const button = screen.getByRole('button', { name: 'Transcend for 1 point' }) as HTMLButtonElement
