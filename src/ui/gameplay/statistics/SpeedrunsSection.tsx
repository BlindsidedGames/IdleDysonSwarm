...
export function SpeedrunsSection({ run, locale, onClearBest }: { readonly run?: SpeedrunStatistics; readonly locale: EnabledLocale; readonly onClearBest?: (milestone: SpeedrunMilestoneId) => Promise<boolean> }) {
  const intl = useIntl()
  const [clearing, setClearing] = useState<SpeedrunMilestoneId | null>(null)
  ...
  return <section className="statistics-speedruns" aria-label={intl.formatMessage(messages.speedruns)}>
    ...
    <div className="statistics-speedruns__milestones">
      {SPEEDRUN_MILESTONES.map(id => {
        ...
        const best = run?.personalBests?.[id]
        const bestLabel = best && isUnboostedSpeedrun(best) ? messages.unboostedBest
          : best && (best.botBoostUsed || best.doubleIpUsed || best.storedTime === 'yes') ? messages.boostedBest : messages.personalBest
    ...
    {clearing && onClearBest && <ClearSpeedrunBestDialog milestone={intl.formatMessage(labels[clearing])}
      onConfirm={() => onClearBest(clearing)} onClose={() => setClearing(null)} />}
