...
public class SidePanelManager : MonoBehaviour
{
    ...
    {
        var show = oracle.saveSettings.prestigePlus.points >= 1 || dvpd.infinityPoints >= 1 ||
                   oracle.saveSettings.unlockAllTabs;
        reality.SetActive(show);
        if (!show) return;
        var unlocked = oracle.saveSettings.prestigePlus.points >= 1 || dvpd.secretsOfTheUniverse >= 27 ||
                       oracle.saveSettings.unlockAllTabs;
    ...
    private void HandlePrestige()
    {
        ...
        if (!show) return;
        var unlocked = oracle.saveSettings.prestigePlus.points >= 1 || dvpd.infinityPoints >= 42 ||
                       oracle.saveSettings.unlockAllTabs;
    ...
    private void HandleInfinity()
    {
        ...
        var unlocked = dvpd.infinityPoints >= 1 || oracle.saveSettings.prestigePlus.points >= 1 ||
                       oracle.saveSettings.unlockAllTabs;
        ...
        else
        {
            var amountForNextPoint =
                BuyMultiple.BuyX(StaticMethods.InfinityPointsToGain(amount, dvid.bots) + 1, amount, 1.2f, 0);

            percent = (dvid.bots -
                       BuyMultiple.BuyX(StaticMethods.InfinityPointsToGain(amount, dvid.bots), amount, 1.2f, 0)) /
