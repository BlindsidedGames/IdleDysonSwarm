...
public class SkillTreeManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            var colours = skillButton.colors;
        ...
        if (oracle.saveSettings.skillsBuyOnTap)
        {
            ...
            else
            {
                if (oracle.SkillTree[skillKey].RequiredSkill != null)
                    foreach (var variable in oracle.SkillTree[skillKey].RequiredSkill)
                        if (!oracle.SkillTree[variable].Owned)
                            available = false;
                if (oracle.SkillTree[skillKey].ShadowRequirements != null)
                    foreach (var variable in oracle.SkillTree[skillKey].ShadowRequirements)
                        if (!oracle.SkillTree[variable].Owned)
                            available = false;
            ...
            if (oracle.SkillTree[skillKey].ExclusvieWith is { Length: >= 1 })
                foreach (var skill in oracle.SkillTree[skillKey].ExclusvieWith)
    ...
    {
        foreach (var variable in oracle.SkillTree)
        {
            var refundable = variable.Value.Refundable;
            if (variable.Value.UnrefundableWith != null)
                foreach (var skill in variable.Value.UnrefundableWith)
                    if (oracle.SkillTree[skill].Owned)
                        refundable = false;
    ...
    private void Clicked()
    {
        switch (oracle.saveSettings.skillsBuyOnTap)
        {
            ...
            case false:
            {
                var stcm = oracle._skillTreeConfirmationManager;
                ...
                if (oracle.SkillTree[skillKey].Owned)
                {
                    foreach (var skill in oracle.allSkillTreeManagers)
                        if (oracle.SkillTree[skill.skillKey].Owned &&
                            oracle.SkillTree[skill.skillKey].RequiredSkill != null && skill.skillKey != skillKey)
                            foreach (var requiredSkill in oracle.SkillTree[skill.skillKey].RequiredSkill)
    ...
    public void MakeLines()
    {
        if (linesMade) return;
        if (oracle.SkillTree[skillKey].RequiredSkill != null)
            foreach (var id in oracle.SkillTree[skillKey].RequiredSkill)
            foreach (var skill in oracle.allSkillTreeManagers)
            {
                if (skill.skillKey == id)
                {
                    var line = Instantiate(linePrefab, oracle.lineHolder).GetComponent<LineManager>();
