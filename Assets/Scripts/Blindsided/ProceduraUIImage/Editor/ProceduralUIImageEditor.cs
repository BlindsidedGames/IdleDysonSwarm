...
namespace Blindsided.ProceduralUIImage.Editor {
    [CustomEditor(typeof(ProceduralUIImage), true)]
    [CanEditMultipleObjects]
    public class ProceduralUIImageEditor : ImageEditor {
        ...
        protected override void OnEnable() {
            foreach (Object obj in serializedObject.targetObjects) {
                if (obj is ProceduralUIImage image) {
                    image.UpdateSerializedValuesFromSharedMaterial();
                }
            }
        ...
        private void AdditionalShapeDataGUI() {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                strokeWidth =
                    ProceduralUIImageEditorGUILayoutExtended.FloatFieldExtended(line, spStrokeWidth.floatValue, dragZone);
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                falloff =
                    ProceduralUIImageEditorGUILayoutExtended.FloatFieldExtended(line, spFalloffDistance.floatValue, dragZone);
                EditorGUI.showMixedValue = false;
            }
            if (EditorGUI.EndChangeCheck())
            {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                outlineWidth =
                    ProceduralUIImageEditorGUILayoutExtended.FloatFieldExtended(line, spOutlineWidth.floatValue, dragZone);
                EditorGUI.showMixedValue = false;
            }
            if (EditorGUI.EndChangeCheck())
            {
        ...
        private void RotationGUI() {
            ...
            EditorGUI.BeginDisabledGroup(spConstrainRotation.boolValue);
            {
                ...
                EditorGUI.BeginChangeCheck();
                {
                    ...
                    rotationValue = ProceduralUIImageEditorGUILayoutExtended.FloatFieldExtended(line, spShapeRotation.floatValue, dragZone);
                    EditorGUI.showMixedValue = false;
                }
                if (EditorGUI.EndChangeCheck()) {
            ...
            EditorGUI.BeginDisabledGroup(!spConstrainRotation.boolValue);
            {
                ...
                line.width = 30;
                line.height = 24;
                if (GUI.Button(line, ProceduralUIImageEditorContents.RotateLeftNormal)) {
                ...
                line.x += 34;
                if (GUI.Button(line, ProceduralUIImageEditorContents.RotateRightNormal)) {
                    float rotation = spShapeRotation.floatValue;
                    float remainder = rotation % 90;
                    if (Mathf.Abs(remainder) <= 0) {
                        rotation -= 90;
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                flipH = GUI.Toggle(line, spFlipHorizontal.boolValue, spFlipHorizontal.boolValue ? ProceduralUIImageEditorContents.FlipHorizontalActive : ProceduralUIImageEditorContents.FlipHorizontalNormal, "button");
                line.x += 34;
                flipV = GUI.Toggle(line, spFlipVertical.boolValue, spFlipVertical.boolValue ? ProceduralUIImageEditorContents.FlipVerticalActive : ProceduralUIImageEditorContents.FlipVerticalNormal, "button");
                EditorGUI.EndDisabledGroup();
            }
            if (EditorGUI.EndChangeCheck()) {
        ...
        private void UpdateShaderChannelWarning() {
            Canvas canvas = (target as ProceduralUIImage)?.GetComponentInParent<Canvas>();
            shaderChannelsNeedUpdate = canvas != null && !HasAdditionalShaderChannels(canvas);
        }
        ...
        private void FixShaderChannelGUI() {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Fix", GUILayout.Width(100))) {
                    Canvas canvas = (target as ProceduralUIImage)?.GetComponentInParent<Canvas>();
                    if (canvas != null) {
                        AddAdditionalShaderChannelsToCanvas(canvas);
        ...
        private static void AddAdditionalShaderChannelsToCanvas(Canvas canvas) {
        ...
        private static bool HasAdditionalShaderChannels(Canvas canvas) {
            AdditionalCanvasShaderChannels channels = canvas.additionalShaderChannels;
            return (channels & AdditionalCanvasShaderChannels.TexCoord1) != 0 &&
        ...
        private void SharedMaterialGUI() {
            ...
            if (EditorGUI.EndChangeCheck()) {
                ...
                foreach (Object obj in targets) {
                    ((ProceduralUIImage)obj).MaterialMode = (MaterialMode)matSett;
            ...
            EditorGUI.BeginDisabledGroup(spMaterialSettings.enumValueIndex == (int)MaterialMode.Dynamic);
            {
                ...
                if (EditorGUI.EndChangeCheck()) {
                    ...
                    foreach (Object obj in targets) {
                        ((ProceduralUIImage)obj).material = (Material)matObj;
                        EditorUtility.SetDirty(obj);
                    }
                }
                ...
                EditorGUI.BeginDisabledGroup(spMaterial.objectReferenceValue != null);
                {
                    if (GUI.Button(new Rect(rect.x + rect.width - 55, rect.y, 55, EditorGUIUtility.singleLineHeight),
                        "Create")) {
                        Material mat = ((ProceduralUIImage)target).CreateMaterialAssetFromComponentSettings();
                        spMaterial.objectReferenceValue = mat;
                        foreach (Object obj in targets) {
                            ((ProceduralUIImage)obj).material = mat;
                            EditorUtility.SetDirty(obj);
                        }
                    }
                }
