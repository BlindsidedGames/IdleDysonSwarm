...
public class InceptionController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        realityInactive.color = oracle.saveSettings.saveData.workersReadyToGo >= 128 ? color_Ready : color_White;
        RunWorkers();
    ...
    {
        var amountWhileAway = (int)Math.Round(time * workerGenerationSpeed);
        ...
        switch (oracle.saveSettings.saveData.workerAutoConvert)
        {
            case true:
                ...
                break;
            case false:
                var total = oracle.saveSettings.saveData.workersReadyToGo + amountWhileAway;
                if (total >= 128)
                {
