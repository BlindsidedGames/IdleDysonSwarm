...
public class SpaceAgeManager : MonoBehaviour
{
    ...
    {
        var solarPanelEnergy = sd1.solarPanels * sd1.solarPanelGeneration;
        if (sd1.mathematicsComplete) solarPanelEnergy *= 2;
        var fusionEnergy = sd1.fusion * sd1.fusionGeneration;
        var dysonPanelEnergy = sd1.swarmPanels * sd1.swarmPanelGeneration;
        var doubleTimeMulti = sdp.doDoubleTime ? oracle.saveSettings.sdPrestige.doubleTimeRate + 1 : 1;
    ...
    private void SpaceFactoryManagement()
    {
        ...
        var multi = 1 + (sd1.spaceFactories > 0 ? Math.Log10(sd1.spaceFactories) : 0);
    ...
    private void RailgunManagement()
    {
        if (sd1.energy > 0 && sd1.railgunCharge < sd1.railgunMaxCharge)
        {
            ...
            var energyTillFill = sd1.railgunMaxCharge - sd1.railgunCharge;
    ...
    private void FireRailGun()
    {
        ...
        var deltaCalc = _timesToFire / _TotalfireTime;
        var timeToFill = _TotalfireTime / _timesToFire;
        _fireTime += deltaCalc * Time.deltaTime;
        var fill = _fireTime / timeToFill;
