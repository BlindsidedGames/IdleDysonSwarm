...
public class InfinityManager : MonoBehaviour
{
    ...
    private void Update()
    {
        UpdateSecret();
        pointsdisplay.text =
            $"Infinity Points: <color=#FFA45E>{dvpd.infinityPoints - dvpd.spentInfinityPoints}<size=70%><color=#91DD8F>({dvpd.spentInfinityPoints})";

        var hasPoints = dvpd.infinityPoints - dvpd.spentInfinityPoints >= 1;
        ...
        txt_purchasedSecrets.text = dvpd.secretsOfTheUniverse >= 1 ? $"Purchased: {dvpd.secretsOfTheUniverse}" : "";
        txt_purchasedSkills.text = dvpd.permanentSkillPoint >= 1 ? $"Purchased: {dvpd.permanentSkillPoint}" : "";
